...
def convert(input_file, keep_additional_info, gl_2_clicks, non_zero_features_only, add_dummy_rank_column = True):
    ...
    # ml4ir expects a pre-ranking. Adding a dummy pre-ranking to match format.
    if add_dummy_rank_column:
        df['rank'] = 1
    return df

def ranklib_to_csv(input_file, output_file, keep_additional_info, gl_2_clicks, non_zero_features_only, add_dummy_rank_column = True):
    ''' Convert the input file with the specified parameters into ml4ir format
        writes the converted file to a csv
    '''

    df = convert(input_file, keep_additional_info, gl_2_clicks, non_zero_features_only)
...
if __name__ == "__main__":
    ...
    ranklib_to_csv(args.input_file, args.output_file, args.keep_additional_info, args.gl_2_clicks, args.non_zero_features_only)
