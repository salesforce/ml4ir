...
class SequenceExampleParserTest(unittest.TestCase):
    ...
    def test_features_spec(self):
        ...
        assert len(set(self.feature_config.get_context_features("name"))
                   ) == len(features_spec[0])
        assert len(set(self.feature_config.get_sequence_features("name"))
                   ) == len(features_spec[1])
    ...
    def test_extract_features_from_proto(self):
        ...
        """
        context_features, sequence_features = self.parser.extract_features_from_proto(
            self.proto)
        ...
        for feature in self.feature_config.get_sequence_features("name"):
            ...
            assert isinstance(
                sequence_features[feature], tf.sparse.SparseTensor)

            feature_tensor = tf.sparse.to_dense(
                tf.sparse.reset_shape(sequence_features[feature]))
    ...
    def test_generate_and_add_mask(self):
        ...
        sparse_rank_tensor = tf.SparseTensor(
            indices, values, rank_tensor.shape)
