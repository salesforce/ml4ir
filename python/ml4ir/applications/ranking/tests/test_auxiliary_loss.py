...
class TestDualObjectiveTraining(unittest.TestCase):
    ...
    def test_end_to_end_aux_one_hot_cross_entropy(self):
        ...
        assert np.isclose(float(results_dict["train_loss"]), 1.19440364, atol=0.0001)
        assert np.isclose(float(results_dict["train_primary_loss"]), 1.196003, atol=0.0001)
        assert np.isclose(float(results_dict["train_aux_loss"]), 1.1880052, atol=0.0001)

        assert np.isclose(float(results_dict["val_loss"]), 1.2132972, atol=0.0001)
        assert np.isclose(float(results_dict["val_primary_loss"]), 1.2164770, atol=0.0001)
        assert np.isclose(float(results_dict["val_aux_loss"]), 1.2005789, atol=0.0001)
    ...
    def test_end_to_end_aux_softmax_cross_entropy(self):
        ...
        assert np.isclose(float(results_dict["train_loss"]), 1.1987438, atol=0.0001)
        assert np.isclose(float(results_dict["train_primary_loss"]), 1.19845056, atol=0.0001)
        assert np.isclose(float(results_dict["train_aux_loss"]), 1.19991695, atol=0.0001)

        assert np.isclose(float(results_dict["val_loss"]), 1.2203352, atol=0.0001)
        assert np.isclose(float(results_dict["val_primary_loss"]), 1.21972501, atol=0.0001)
        assert np.isclose(float(results_dict["val_aux_loss"]), 1.2227761, atol=0.0001)