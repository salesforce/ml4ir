...
class FeatureLayerTest(RelevanceTestBase):
    ...
    def test_query_embedding_vector_using_glove(self):
        # Setup mock feature_info and file_io
        feature_info = {
        ...
        # Mock the GloVe file content
        glove_content = "word1 0.1 0.2 0.3\nword2 0.4 0.5 0.6\nword 0.9 0.9 0.9"
        open_mock = unittest.mock.mock_open(read_data=glove_content)
        with unittest.mock.patch('builtins.open', open_mock):
            query_embedding_vector = string_transforms.QueryEmbeddingVector(feature_info, file_io)
        word1 = tf.constant("word1", dtype=tf.string)
        word2 = tf.constant("word2", dtype=tf.string)

        # assert dimensions
        self.assertIn(str(word1), query_embedding_vector.word_vectors)
